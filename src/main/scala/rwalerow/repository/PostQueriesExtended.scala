...
class PostQueriesExtended(discussions: TableQuery[Discussions])(implicit override val db: JdbcProfile#Backend#Database,
  ...
  def countBefore(discussionId: Long, date: Timestamp): Future[Int] =
    db.run(countPostsBeforeDateC(discussionId, date).result)
  def countAfter(discussionId: Long, date: Timestamp): Future[Int] =
    db.run(countPostsAfterDateC(discussionId, date).result)
  ...
  private def countBeforeR(discussionId: Rep[Long], date: Rep[Timestamp]): Rep[Int] =
    countByQ(discussionId, _.createDate < date)
  private def countAfterR(discussionId: Rep[Long], date: Rep[Timestamp]): Rep[Int] =
    countByQ(discussionId, _.createDate > date)
  ...
  def findInRange(takeBefore: Int, takeAfter: Int, postIndex: Long = 0, discussionId: Long): Future[Seq[Post]] =
    db.run {
      tableQuery
        .filter(_.discussionId === discussionId)
        .sortBy(_.createDate)
  ...
  def deletePost(discussionId: Long, secret: Secret): Future[Int] =
    deleteByFilter(x => x.discussionId === discussionId && x.secret === secret)
}
