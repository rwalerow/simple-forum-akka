...
class PostQueriesExtended(implicit override val db: JdbcProfile#Backend#Database, implicit override val profile: JdbcProfile)
  ...
  def countBy(discusisonId: Long, f: Posts => Rep[Boolean]): Future[Int] = db.run {
    tableQuery.filter(_.discussionId === discusisonId).filter(f).countDistinct.result
  }

  def before(discussionId: Long, date: Timestamp): Future[Int] = countBy(discussionId, _.createDate < date)
  def after(discussionId: Long, date: Timestamp): Future[Int] = countBy(discussionId, _.createDate > date)

  def postWithIndex(discussionId:Long, id: Long): Future[Option[(Post, Long)]] = db.run {
    tableQuery.filter(_.discussionId === discussionId).sortBy(_.createDate).zipWithIndex.filter{ case (p, i) => p.id === id }.result.headOption
  }

  def findInRange(takeBefore: Int, takeAfter: Int, postIndex: Long = 0, discussionId: Long): Future[Seq[Post]] = db.run {
    tableQuery.filter(_.discussionId === discussionId).sortBy(_.createDate).zipWithIndex.filter{
